...
def login():
    ...
    if request.method == "POST":
        ...
        if users.login(username, password):
            ...
            #'''
            return render_template("index.html", error=True, message=f"Username {username} or Password {password} is incorrect", username=username, password=password)
            #'''
            '''
            return render_template("index.html", error=True, message=f"Username or Password is incorrect", username=username, password=password)
            '''
...
def register():
    ...
    if request.method == "POST":
        ...
        if already_user:
            return render_template("register.html", error=True, message="Username is already taken", username=username, password1=password1, password2=password2)

        if len(username) < 3:
            return render_template("register.html", error=True, message="The username must be longer than three characters", username=username, password1=password1, password2=password2)

        if " " in username:
            return render_template("register.html", error=True, message="The username cannot contain spaces", username=username, password1=password1, password2=password2)
...
def participate_event():
    ...
    if csrf_token == valid_csrf_token:
        ...
        if already_participate:
            print("The user is already registered as a participant")
            participation_status = events.get_participation(user_id, event_id)
            messages = events.get_messages_by_event(event_id)
            return render_template("event_info.html", messages=messages, participation_status=participation_status, event=event, participation=True, message="You have already registered to attend the event", participants=participants, count=count)
        participate = events.participate_event(user_id, username,  event_id)
        print("Osallistuminen onnistui")
        participation_status = events.get_participation(user_id, event_id)
        messages = events.get_messages_by_event(event_id)
        return render_template("event_info.html", messages=messages, participation_status=participation_status, event=event, message="Your registration as a participant for the event was successful!", participants=participants, count=count)
...
def modify_event_info():
    ...
    if csrf_token == valid_csrf_token:
        ...
        participation_status = events.get_participation(user_id, event_id)
        messages = events.get_messages_by_event(event_id)
        return render_template("event_info.html", messages=messages, participation_status=participation_status,  event=event, modified=True, message="Changes saved successfully", participants=participants, count=count)
...
def delete_event():
    ...
    if csrf_token == valid_csrf_token:
        ...
        events_list = events.get_all()
        count = len(events_list)
        return render_template("index.html", events=events_list, count=count, deleted=True, message="The event has been successfully deleted")
...
def delete_participation():
    ...
    if csrf_token == valid_csrf_token:
        ...
        participation_status = events.get_participation(user_id, event_id)
        messages = events.get_messages_by_event(event_id)
        return render_template("event_info.html", messages=messages, participation_status=participation_status, event=event, count=count, deleted=True, message="Participation canceled", participants=participants)
